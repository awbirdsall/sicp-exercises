...
; but change make-rat so it does not attempt to reduce fractions
; to lowest terms.
; ex 2.94: implement remainder-terms (using div-terms) and use
; this to define gcd-terms. Write procedure gcd-poly that computes
; poly gcd of two polys (return error if diff variable). Install
; generic greatest-common-divisor that handles polys or normal numbers.
; ex 2.95: notice weird behavior if we use polynomials that lead to
; non-integer coefficients using the gcd algorithm, e.g.,
; (define p1 (make-polynomial 'x (make-sparse-termlist (list (make-sparse-term 2 (make-integer 1))
;                                                            (make-sparse-term 1 (make-integer -2))
;                                                            (make-sparse-term 0 (make-integer 1))))))
; (define p2 (make-polynomial 'x (make-sparse-termlist (list (make-sparse-term 2 (make-integer 11))
;                                                            (make-sparse-term 0 (make-integer 7))))))
; (define p3 (make-polynomial 'x (make-sparse-termlist (list (make-sparse-term 1 (make-integer 13))
;                                                            (make-sparse-term 0 (make-integer 5))))))
; (define q1 (mul p1 p2))
; (define q2 (mul p1 p3))
...
(define (install-dense-termlist)
  ...
  (define (pseudoremainder-terms L1 L2)
    (let (int-factor (exp (dense-coeff (first-term L2) (+ 1 (- (dense-order (first-term L1))
                                                               (dense-order (first-term L2)))))))
      (let (pseudo-L1 (mul int-factor L1))
...
(define (install-sparse-termlist)
   ...
   ;;
   ;; sample usage:
   ;; (make-sparse-termlist (list (make-sparse-term 1 (make-integer 3))
   ;;                             (make-sparse-term 0 (make-integer 1))))
  (define (adjoin-term term term-list)
...
; ex 2.93: modify rational-arithmetic to use generic operations.
; Allows making rational polynomials.
(define (install-rational-package)
  ;; internal procedures
  ;; rat is cons of numerator n and denominator d
