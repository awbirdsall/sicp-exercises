...
; ex 2.92: impose ordering on variables so that polynomials can be
; converted to a 'canonical form' and addition/multiplication of
; polynomials in different variables is possible ("this is not easy!")

; helper procedure used by compare-symbol-termlist in termlist package,
; defines ordering of variables based on `string<?` built-in.
(define (higher-symbol s1 s2)
...
(define (compare-symbol-termlist L symbol)
  (apply-generic 'compare-symbol-termlist L symbol))
; dispatch highest-symbol within polynomial pacakge, with call to
; compare-symbol-termlist that dispatches to termlist package
(define (highest-symbol p) (apply-generic 'highest-symbol p))
...
(define (install-polynomial-package)
  ...
  (put 'highest-symbol '(polynomial) highest-symbol)
  'done)
...
(define (install-dense-termlist)
  ...
  (define (compare-symbol-termlist L base-symbol)
    ...
    (define (compare-symbol-termlist-iter working-L result)
      (cond ((null? working-L) result)
            ((not (eq? 'polynomial (type-tag (dense-coeff (first-term working-L)))))
             (compare-symbol-termlist-iter (rest-terms working-L) result))
            (else (compare-symbol-termlist-iter (rest-terms working-L)
                                       (higher-symbol result (variable (dense-coeff (first-term working-L))))))))
  ...
  (put 'compare-symbol-termlist '(dense-termlist symbol) compare-symbol-termlist)
  'done)
...
(define (install-sparse-termlist)
  ...
  (define (compare-symbol-termlist L base-symbol)
    ...
    (define (compare-symbol-termlist-iter working-L result)
      (cond ((null? working-L) result)
            ((not (eq? 'polynomial (type-tag (sparse-coeff (first-term working-L)))))
             (compare-symbol-termlist-iter (rest-terms working-L) result))
            (else (compare-symbol-termlist-iter (rest-terms working-L)
  ...
  (define (expand-sparse-termlist L)
    ...
    (define (expand-term t1)
      ; takes a single term t1 of termlist L and returns a list of terms, each of order (sparse-order t1),
      ; generated by unpacking the "inner-termlist" of the term-list within the polynomial coeff of t1
      ; and wrapping each as the single-term poly coeff of a term of order (sparse-order t1).
      (define (not-poly-coeff? x)
        (not (eq? (type-tag (sparse-coeff x)) 'polynomial)))
      (if (not-poly-coeff? t1)
          ; #t means t1 is terminating leaf, assuming no weird nesting of polynomial within non-polynomial coeff
          (list t1)
...
; ex 2.92: add support for dispatch on symbol using symbol?
; primitive (for highest-type procedure on termlist and symbol)
